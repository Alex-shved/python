...
class Cell:
    ...
    def __mul__(self,mult):
        print('=' * 5, 'mul', '=' * 5)
        return '*' * (len(self.num) * mult)

    def __truediv__(self,div):
        print('=' * 5, 'truediv', '=' * 5)
        return '*' * (len(self.num) // div)


def make_order(cls, nums):
    print('=' * 10)
    return '\n'.join(['*' * nums for _ in range(len(cls.num) // nums)]) + '\n' + '*' * (len(cls.num) % nums)
